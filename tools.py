...
def getLocation(requestIP):
    try:
        ...
        print "HTTP Exception occured. Can't get the location of the submitter."
#        print e
...
def getMarkers(locations):
    ...
    """
    if not locations: #(location is None) or (len(locations) <=0):
        return ""#todo
    base = "markers="
...
def getMapURL(locations, key= API_KEY, url_base = URL_BASE, width="400", height="400"):
    if (not locations): #This covers (location is None) or (len(locations) <= 0):
        return ""
