...
#    print "Length of cache: ", len(CACHE.get(key,[]))
    if not update and key in CACHE:
        ...
        arts = list(Art.gql("ORDER BY created DESC LIMIT 10"))
...
class Handler(webapp2.RequestHandler):
    ...
    def write_front(self, title="", art="", error_title="", error_art=""):
        arts = get_top10()
        logging.info('number of arts on the front: %d'%len(arts))
#        for a in arts:
#            print a.location
        mapURL = None
        if arts:
            ...
            if locations:
                ...
                print "number of pins: %d"%len(locations)
#        if arts: #debug
